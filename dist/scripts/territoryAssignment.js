...
let zoneArrayP1 = [];
let zoneArrayP2 = [];
const floodFill = (board, pieces, x, y, target, replacement) => {
    ...
    if (x !== 0) {
        b = floodFill(b, pieces, x - 1, y, target, replacement);
    }
    if (x !== 0 && y !== 0) {
        b = floodFill(b, pieces, x - 1, y - 1, target, replacement);
    }
    if (y !== 0) {
        b = floodFill(b, pieces, x, y - 1, target, replacement);
    }
    if (x !== 0 && y !== 9) {
        b = floodFill(b, pieces, x - 1, y + 1, target, replacement);
    }
...
exports.territoryAssignment = (grids, pieces) => {
    ...
    const newStructureGrid = StructureGrid.map((row, y) => row.map((block, x) => {
        const p1Val = t1[y][x];
        const p2Val = t2[y][x];
    ...
    return {
        removablePieces,
        grids: Object.assign({}, grids, { StructureGrid: newStructureGrid }),
    };
};
const configureFloodFill = (b, p) => {
    const floodFill = (x, y, target, replacement) => {
        ...
        if (x !== 0 && y !== 0) {
            floodFill(x - 1, y - 1, target, replacement);
        }
        if (y !== 0) {
